...
在目标的认知和管理中，除了围绕对象本身的下探，还可以关注直接和间接影响目标的其他对象或指标。例如，需求交付周期（RDC）与需求颗粒度（RG）之间存在关联，即：$\overline{RDC}=f(\overline{RG})$，其中 $f()$ 表示一定的函数关系。所以，我们在认知和关注需求交付周期时应同时关注需求颗粒度。关联可能存在于两个指标之间，也可能存在于多个指标之间，我们可以统一为如下的问题。
