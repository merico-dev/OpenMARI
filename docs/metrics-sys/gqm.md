...
为了认知和管理目标，我们需要围绕目标的对象和维度深入提问。相应地，问题模板以 `QT-<P><O><T>` 格式编码。其中，`<P>` 代表维度（perspective）字母，标记围绕维度的提问角度；`<O>` 代表对象（objective）字母，标记围绕对象的提问角度；`<T>` 代表时间（time）字母，标记针对时间周期的提问角度。

提问的第一步应从目标的维度入手，首先对维度的概念进行明确或拆解。例如，目标是“认知软件的可靠性”，那么“可靠性”这个概念怎么定义呢？如果希望用软件交付后的缺陷数量来衡量可靠性，那么问题围绕的**具体对象**应为软件交付后的缺陷，针对的**具体维度**应为其数量。所以，目标中的维度可能被定义成一个或多个具体维度来设计问题，包括如下三类情况。
...
- 问对象同类，标记为 `C`（congeneric）。与同类对象比较，是建立“参照系”的一类典型做法。进行对比的“同类对象”可以是具体的一个集合，也可以是特定集合上抽象的统计值（如组织的基线、行业的基线等），还可以是同一个对象在不同周期整体的实例（如一个对象计算环比时可视为两个同类对象间的比较）。例如：作为项目经理关注的目标，当前外包项目的缺陷密度与<u>同类其他外包项目</u>比怎么样？
- 问对象构成，标记为 `P`（parts）。有些对象具备层级结构（如团队包含子团队、项目包含子项目）或由若干部分构成（如需求交付过程包含若干阶段、软件包含若干模块），那么可以对目标对象的构成提问，通常为了分析主因或瓶颈。例如：部门内<u>各团队</u>的代码产出量怎么样？需求交付过程<u>各阶段</u>耗时多久？
...
| | 单一维度 | 组合维度 | 关联维度 |
| --- | --- | --- | --- |
| 对象整体 | `QT-SOF` `QT-SOP` | `QT-COF` `QT-COP` | `QT-ROF` `QT-ROP` |
| 同类对象 | `QT-SCF` `QT-SCP` | `QT-CCF` | `QT-RCF` |
| 对象构成 | `QT-SPF` `QT-SPP` | `QT-CPF` | `QT-RPF` |

### 问题列表及视图

对上述各类问题的回答主要以指标度量的数据视图（即图表）为载体，我们在此一并列出，方便更具像化的理解。问题模板的表述比较技术化，主要为了反映问题实质。在实际应用中，它们可转换成更容易被开发团队理解的表述。

#### `QT-SOF`：<对象>的<指标>是多少？ {#QT-SOF}

这是最直接的提问方式，适用于简单易懂的指标。表述中未提及时间要素，默认指周期整体。以“需求吞吐量”为例，如果开发团队都了解这个概念，那么可以直接按模板提问，即“项目的需求吞吐量是多少”；否则，可转述成“项目近期交付的需求有多少”。

📈 典型视图：单一数值；其他视图上的辅助线。

#### `QT-SOP`：<对象>的<指标>近期怎么样？ {#QT-SOP}

认知包含了解和评价，而 `QT-SOF` 类问题的回答是单一数值，可供“了解”，但缺乏“参照系”，无法形成数值高低的概念或判断（即“评价”），在实际当中的可用性往往打折扣。从时间维度看指标变化，是建立“参照系”的一种典型做法。我们可以跟自己比，看指标在历史上或最近处于高位还是低位。

例如，“该项目各迭代开发当量的离散系数近期怎么样？”——或转述为“该项目各迭代开发产出是否稳定？”

📈 典型视图：折线图或柱状图，纵轴为指标值，横轴为时间（按特定步长）。

#### `QT-SCF`：<对象>的<指标>与{同类对象}比怎么样？ {#QT-SCF}
...
- `QT-SOP`$\Rightarrow$`QT-SOF`/`QT-COF`：在时间维度上综合提问。例如，在回答问题“该项目近期新增代码当量有多少”（[`QT-SOP`](#QT-SOP)）的视图上，我们可以进一步提问“该项目每周新增代码当量的离散系数是多少”（[`QT-SOF`](#QT-SOF)）来评估开发产出稳定性，呈现为视图上标记的一个数值。
- `QT-SCF`/`QT-SPF`$\Rightarrow$`QT-SOF`/`QT-COF`：将多个对象在单一维度上综合提问。例如，在回答“该项目各迭代的需求交付周期是多少”（[`QT-SPF`](#QT-SPF)）的视图上，我们可以进一步提问“该项目各迭代需求交付周期的上限和下限是多少”（[`QT-COF`](#QT-COF)）作为各个迭代数值的参考，呈现为视图上的一个辅助线。
- `QT-CCF`/`QT-CPF`$\Rightarrow$`QT-COF`：将多个对象分别在组合的各个维度上综合。例如，问题“各位开发者的代码在几个关键质量指标上表现如何”（`QT-CPF`）的数据视图可以呈现为雷达图，其上可以标记出回答“他们的代码在各个关键质量指标上的均值是多少”（`QT-COF`）的折线作为参照。
- 另有一类分析不改变问题形式但从每个数据点上计算出新的值，我们也归入综合类。例如数据点之间的比值或者各自占总数的比例：问题“各团队开发者人数是多少”（`QT-SPF`）的视图上可以标记出“各团队开发人数占多大比例”（`QT-SPF`）。

我们不推荐在较复杂的视图上再综合其他问题，会导致视图信息过载，不便于用户理解。例如，`QT-SCP` 和 `QT-SPP` 同时包含了多个对象和时间变化，再添加辅助线会降低图表可读性。同理，关联维度类问题如果综合多个对象，在视图上不易区分综合结果对应哪个维度。
...
- 聚焦于某个对象，将对象编码由 `C` 或 `P` 变为 `O`；
- 聚焦于某个维度，将维度编码由 `C` 或 `R` 变为 `S`。

三种聚焦方式可同时应用。例如，[`QT-ROP`](#QT-ROP)&rarr;[`QT-SOF`](#QT-SOF)同时变换了时间编码和维度编码：在回答“项目的需求吞吐量近期怎么样？与需求颗粒度的关系如何？”（[`QT-ROP`](#QT-ROP)）的视图上，可以选取某一时段的需求吞吐量或者需求颗粒度，以呈现具体信息，那么相应的问题分别是“项目在该时段的需求吞吐量是多少”或“项目在该时段的需求颗粒度是多少”（[`QT-SOF`](#QT-SOF)）。

结合实际场景，聚焦后的问题应属于如下模板之一：
- `QT-SOF`：聚焦一个对象特定时段的某个维度；
- `QT-SOP`：聚焦一个对象各个时段的某个维度
- `QT-SCF`/`QT-SPF`：聚焦多个对象特定时段的某个维度；
- `QT-COF`：聚焦一个对象特定时段的多个维度。
...
- 基于时间下探：
  - `QT-SOF`$\rightarrow$`QT-SOP`
  - `QT-SCF`$\rightarrow$`QT-SCP`
  - `QT-SPF`$\rightarrow$`QT-SPP`
  - `QT-COF`$\rightarrow$`QT-COP`
- 基于对象下探：
  - `QT-SOF`$\rightarrow$`QT-SPF`
  - `QT-SOP`$\rightarrow$`QT-SPP`
  - `QT-COF`$\rightarrow$`QT-CPF`
- 基于维度下探：
  - `QT-SOF`$\rightarrow$`QT-COF`
  - `QT-SOP`$\rightarrow$`QT-COP`
  - `QT-SCF`$\rightarrow$`QT-CCF`
  - `QT-SPF`$\rightarrow$`QT-CPF`
