...
对一个问题得到的数据视图整体进行进一步提问，我们称之为综合。下面列出一些典型问题上可进一步提出的综合类问题。
...
在一张数据表中，用户可能会关注到某个元素，希望获得更多信息。这种行为包含了进一步的提问，问题应给予回答，或者作为后续下探的基础。

- 聚焦于某个时段，将时间编码由 `P` 变为 `F`。
- 聚焦于某个对象，将对象编码由 `C` 或 `P` 变为 `O`。
- 聚焦于某个维度，将维度编码由 `C` 或 `R` 变为 `S`。

以上三种聚焦方式可以同时应用。例如，[`QT-ROP`](#QT-ROP)&rarr;[`QT-SOF`](#QT-SOF)同时变换了时间编码和维度编码：在回答“项目的需求吞吐量近期怎么样？与需求颗粒度的关系如何？”（[`QT-ROP`](#QT-ROP)）的视图上，可以选取某一时段的需求吞吐量或者需求颗粒度，以呈现具体信息，那么相应的问题分别是“项目在该时段的需求吞吐量是多少”或“项目在该时段的需求颗粒度是多少”（[`QT-SOF`](#QT-SOF)）。
