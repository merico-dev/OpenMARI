...
文章[^vanSolingen(2002)]主张区分度量目标和改进目标。
度量为改进行动提供所需信息，即哪里需要改进和如何进行改进。
实际上，组织最终关心的是改进目标，度量目标为改进目标服务。
...
问题主要用于刻画目标[^vanSolingen(2002)]，隐含着在目标视角下刻画目标对象所采用的模型。
我们通过一个实例来具体理解“刻画”的含义和常见方式，见表 2。

| 目标 | 研发团队了解已交付产品的可靠性
| --- | --- |
| 问题 1 | 交付后发生的故障在时间上的分布是什么样的？ |
| 问题 2 | 交付后发生的故障在严重级别上的分布是什么样的？ |
| 问题 3 | 交付后发现的缺陷在模块上的分布是什么样的？ |
| 问题 4 | 模块重用性和可靠性之间的关系是什么样的？ |
| 问题 5 | 模块复杂度和可靠性之间的关系是什么样的？ |
| 问题 6 | 失效处理各阶段上的工作量的分布是什么样的？ |

表 2 - 问题列表实例

上面的问题列表并不一定是完备的，可以扩充或裁剪，取决于研发团队实际的信息需要。
...
我们再举一个更为完备的隐式模型的例子。
比如目标是：公司评价某项目测试流程中的缺陷逃逸。
可能的问题包括：
- 在该项目中，系统测试、验收测试以及发布后发现的缺陷分别有多少？
- 在该项目的同类项目中，系统测试、验收测试以及发布后发现的缺陷分别有多少？
- 在该项目中，系统测试发现的缺陷数与系统测试后发现的缺陷数之间的比值是多少？
- 在该项目的同类项目中，系统测试发现的缺陷数与系统测试后发现的缺陷数之间的比值平均是多少？
- 该项目缺陷逃逸水平与同类项目相比怎么样？
