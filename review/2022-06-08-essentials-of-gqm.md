...
- **视点**：用户、开发者、管理者、公司等。
- **环境**：度量的上下文，包括人员、开发模式、工具等，用于判断度量间的可比性。
...
我们注意到，环境这个参数在相关文献中较少展开论述。
它在今天研发效能度量中的主要意义是支持度量间的比较，不论组织内部，还是同行对比。
但是，它对解决问题不直接产生作用，而度量间的可比性在实际中不需要非常精确地界定，不必具体到每个目标。
所以，我们推荐在实践中简化目标定义，忽略目标的环境参数，只需针对要比较的对象考察环境要素。

### 操作层：问题

问题主要用于刻画目标[^vanSolingen(2002)]，表达以目标视角刻画目标对象的模型。
全面的问题包含如下几个方面[^Basili(1992)]：
- 目标对象的属性或行为的定义。
- 目标视角的定义。
- 目标视角下的改进。
...
- **对象**：研发的过程、产物或资源。
- **视角**：目标聚焦的角度或对象的属性。
- **角色**：组织中谁会关注该目标，为开发者、项目经理、技术经理、综合管理者中的一个或多个。
