...
我们今天可以看到关于研发效能度量的很多文章、演讲、案例和书籍。
...
前者适合有研究背景的读者，但其表达和格式上存在一些不足之处，可能影响阅读和理解；后者适合行业中更广泛的读者，但其裁剪和简化也有容易误导实践之处，下面会逐一指出。
...
- **视点**：用户、开发者、管理者、公司等。
- **环境**：度量的上下文，包括人员、开发模式、工具等，用于判断项目间的可比性。

有观点[^vanSolingen(2002)]认为应当区分度量目标和改进目标。
度量目标可由改进目标得出，并为改进行动提供所需信息，即哪里需要改进和如何进行改进。
实际上，组织最终关心的是改进目标，度量目标最终为改进目标服务。
