...
- 我们将目标定义标准化为如下四个要素：
  - **对象**：研发的过程（process）、产物（product）或资源（resource）中的一种。
  - **维度**：目标聚焦的角度或对象的属性。
  这是一个开放的集合，同时由 OpenMARI[^OpenMARI] 对涉及的概念进行具体规范。
  - **目的**：了解（understanding）、评价（evaluation）、改进（improvement）和预测（prediction）中的一种。
...
- 几种目的下，对目标对象的度量分析范围有所不同。

  | 目的 | 当前范围 | 同类范围 | 历史范围 | 下钻范围 |
  | --- | --- | --- | --- | --- |
  | 了解 | ✓ |  |  |  |
  | 评价 | ✓ | ✓ | ✓ |  |
  | 预测 | ✓ |  | ✓ |  |
  ...
  - 了解仅针对目标对象本身和当下，但它是评价、预测和改进的基础，其问题可被三者复用。
  - 评价通过对比而产生，依赖与目标对象可比的同类对象或它们的基准；也可能涉及与历史基准的对比。
  - 预测对历史数据的稳定性要求最高。
  - 改进通常涉及下钻，以分析原因或瓶颈。
...
- 问题是对模型的定义，该模型一方面是对目标的拆解或刻画，另一方面是将数据解读为目标的途径。
...
- 为了规范和简洁，以指标为基础的各类统计数据（如均值、同比增长率等），归入对相应指标的分析，而不是单列为新的指标。
- 在规范化的同时，指标支持根据目标环境定制的参数，例如需求交付周期划分成哪几个阶段、客户满意度调研包含哪几个话题等。
这些参数在指标的度量中指明。
