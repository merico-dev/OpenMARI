...
- 我们将目标定义标准化为如下四个要素：
  - **对象**：研发的过程（process）、产物（product）或资源（resource）中的一种。每种对象都可能存在层级结构。
    - 研发过程下钻，可以到需求、设计、开发、测试、发布、运营等过程；每个过程还可以进一步下钻。
    - 产物包括软件、文档、制品等。以软件为例，可以下钻到组件、函数、缺陷、变更等。
    - 资源包括人员、设备等。人员组成团队，形成层级结构。
  - **维度**：目标聚焦的角度或对象的属性。
  这是一个开放的集合。
  - **目的**：了解（understanding）、改进（improvement）、预测（prediction）和评价（evaluation）中的一种。
  - 角色：组织中谁会关注该目标，通常为开发者（developer）、项目经理（project manager）、技术经理（tech lead）、高层管理者（executive）中的一个或多个。
- 几种目的下，围绕目标对象的问题会有不同的度量分析范围，可总结为[表 5](#purpose-scope)。

  <a name="purpose-scope">表 5</a> - 不同目的对应的度量范围

  | 目的 | 对象本身 | 组成部分 | 历史周期 | 对象同类 |
  | --- | --- | --- | --- | --- |
  | 了解 | ✓ | ✓ | ✓ |  |
  | 改进 | ✓ | ✓ | ✓ |  |
  | 预测 | ✓ |  | ✓ |  |
  ...
  - 了解是改进的基础。
  为了全面了解目标对象，我们除了刻画对象本身，通常也刻画对象的组成部分和历史变化。
  - 改进需要下钻到对象的组成部分以分析原因或瓶颈，也需要与历史对比体现改进的实际效果。
  - 预测要基于历史，对历史数据的稳定性要求高。
  - 评价通过对比产生，需参照与目标对象可比的同类对象或它们的基准。
...
例如，需求多长时间交付的问题，可把需求交付的阶段划分作为参数。
实际应用时，具体阶段划分即定义了当前项目需求交付过程的一个模型。
原问题可具体化为需求交付的每个阶段多长时间，也可视为一组具体问题。
...
- 为了规范和简洁，以指标为基础的各类统计（如均值、同比增长率、分布等），归入对应指标的分析方法，而不单列为另外的指标。
典型的分析方法包括：
  - 数值分布：指标数值作为随机变量，计算其数值的分布，包括分布的统计量，如均值、方差、上下四分位等。
  - 时序分布：按时间顺序分析指标数值，包括时间维度上的均值、方差、同比、环比、趋势等。
  - 分类分布：将对象的组成部分分类后，进行统计分析，如缺陷按优先级分类，统计各优先级缺陷的数量。
  - 同类分布：对象与可比同类进行对照，刻画对象相对于同类的表现。
- 指标回答带有参数的问题时，需要将参数作为分析输入。

## 参考阅读

:::info
引用本文：任晶磊．GQM 从入门到精通．_研发效能评论_，2022-06-08．https://ls.meri.co/55bb4126
:::
